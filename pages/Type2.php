...
<head>
    ...
    <title>Book Your Stay at RUKSABAI | Room Type 2</title>
    <style>
    <style>
      .booking-modal {
        position: fixed;
      ...
      <div class="info-box">
          ...
          <form id="roomForm">
              ...
              <select name="room_id" id="room">
                  ...
                  <?php
                  $result = $conn->query("SELECT room_id, room_number, status FROM rooms WHERE room_number IN (201, 202, 203, 204, 205)");
                  while($row = $result->fetch_assoc()) {
                      echo "<option value='{$row['room_id']}'>{$row['room_number']} - {$row['status']}</option>";
                  }
                  ?>
              </select>
